...
## Если по ссылке открывается README вместо сайта

Публичная ссылка GitHub Pages должна открывать приложение, но если Pages настроен на публикацию корня репозитория, GitHub может показать `README.md`. Для такого режима добавлен корневой `index.html`: он автоматически переводит посетителя в папку `site/`, где лежит настоящий сайт.

После merge/push этого файла достаточно подождать несколько минут и открыть:

```text
https://kostaweather.github.io/weathercloud-hourly-export/
```

Если GitHub Pages всё ещё показывает README, откройте `Settings` → `Pages` и убедитесь, что выбран источник `Deploy from a branch`, нужная ветка и папка `/root`, либо переключитесь на `GitHub Actions`.
